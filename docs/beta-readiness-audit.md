...
## Addendum 5 — Youth zipCd structured region eligibility (real fix; closes P1-4)

Started from SHA `f75f8af1495d2159fa8bd0f0620fa814bfade95c`. This checkpoint closes **P1-4** (Youth Center region leakage), the last open item from Addendum 4's list, by finally resolving `zipCd` — previously unwired end-to-end since the original Phase 4-B checkpoint (`ZIP_CD_PROVENANCE`/`ZIP_CD_NEXT_STEP`).

**Root cause confirmed the same as originally suspected**: `zipCd` was never in the official 온통청년 코드정의서 XLSX, so it was left completely unstructured — every Youth record's region condition was "unknown" regardless of the user's actual residence, letting city-specific Youth programs (e.g. 서산시/홍성/아산시-only benefits) leak into other cities' Home Top-10 and full-catalog feeds purely on interest/age/employment overlap.

**Fix**: `zipCd`'s raw 5-digit values are resolved against a DIFFERENT, independently-authoritative source — the Korean government's own 법정동코드 전체자료 dataset — not the XLSX (which genuinely still lacks it; that fact is unchanged). All 261 distinct `zipCd` tokens observed in the frozen 2,745-record Youth snapshot were classified with **zero unmapped tokens** (217 plain current leaf, 39 subordinate-gu-of-a-13-city, 2 parent-aggregate, 3 historical/폐지 Incheon codes) and committed as `domain/youthCodebook/zipCdCrosswalk.ts`'s `YOUTH_ZIPCD_CROSSWALK`. A new `RegionSpec.subdivision` field and `domain/region/subdivisionPartition.ts` prevent the one unsafe shortcut this data shape invites: naively collapsing a subordinate-gu zipCd (e.g. 수원시 장안구) to its whole parent city, which would silently over-grant every resident of the other 3 gu. `domain/youthCodebook/compatibility.ts`'s new `buildYouthRegionRule()` wires the crosswalk into a `region_in` rule (id `youth-region`) in `adapters/youthCenter/YouthAdapter.ts`'s `buildEligibility()`; an unrecognized future token (or any OR-branch failing to resolve) still leaves the whole dimension `hasUnresolvedEligibility` rather than guessing.

**Re-ran `scripts/auditBetaReadiness.ts` against the same frozen catalog** (MOIS 10,967 / Youth 2,745 rows, `docs/audits/beta-readiness-personalization.json` overwritten with the new run):

- **A_icheon_unemployed_youth**: `relevantSuspiciousLocalCount` (Youth records whose title/org names a different region, with zero region evidence) collapsed **545 → 0**; `top20SuspiciousLocalCount` **10 → 0**; the one confirmed Home Top-10 leak (아산시 AI-디스플레이 인력양성사업, a 아산시-only program) is gone. `relevantYouthCount` correctly shrank 831 → 246 (the ~585 genuinely out-of-region Youth records now resolve to `not_eligible` instead of indefinite `unknown`). Home Top-10 still fills (10/10), now backed by real `exact_city`-evidenced Youth/MOIS matches instead of unverified ones.
- **D_icheon_newlywed**: `relevantSuspiciousLocalCount` **476 → 0**. Two previously-leaked, genuinely-wrong-region Youth items (서산시 청년 신혼부부 주택 전세자금 대출이자 지원사업; 홍성 신혼부부 주거자금 대출이자 지원) are confirmed removed from Home Top-10, replaced by two additional `exact_city`-verified 이천시 MOIS benefits.
- **B/C/E**: `not_eligible` counts rose and `unknown` counts fell by a similar magnitude (Youth region conditions now resolve definitively instead of staying perpetually unknown); Home Top-10 fill and `likely_eligible` counts (0 everywhere, structurally expected per P1-3) unchanged.
- **F_minimal_just_onboarded** (no residence at all): completely unchanged (`matchRegion` still correctly returns `unknown` with no residence, so a resolved region rule can never manufacture a FAIL for a residence-less profile) — confirms nationwide/no-residence semantics are preserved exactly.
- Full suite: 1023/1023 tests passing (61 files), 0 typecheck errors, 0 lint errors, production build clean.

**Verdict: `READY_AFTER_SMALL_FIXES` → all P1 items now closed except P1-3.** Final P1 state: **P1-1, P1-2, P1-4, P1-5 all CLOSED.** Remaining open, deliberately not fixed: **P1-3** (`likely_eligible` structurally unreachable — separate, larger checkpoint), P2-3 ("확인이 필요해요" comprehension — beta-only), and the 3 documented MOIS source-data limitations (`mois-648000001072`, `mois-569000000402`, `mois-631000000709`) — none are Youth-region or zipCd issues.
